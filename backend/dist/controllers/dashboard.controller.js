...
export const medicalProfile = async (req, res) => {
...
export const updateProfile = async (req, res) => {
    const { firstName, lastName, diabetesType, age, gender, diagnosisDate, } = req.body;
    const token = req.headers.authorization?.replace("Bearer ", "");
    try {
        const { data: { user }, error, } = await supabase.auth.getUser(token);
        if (!user) {
...
export const updateGlucosePreference = async (req, res) => {
    const { bloodSugarUnit, targetBloodSugarRange, } = req.body;
    const token = req.headers.authorization?.replace("Bearer ", "");
    try {
        const { data: { user }, error, } = await supabase.auth.getUser(token);
        if (!user) {
...
export const addReading = async (req, res) => {
    const { mealTiming, timestamp, value, note } = req.body;
    const token = req.headers.authorization?.replace("Bearer ", "");
    try {
        const { data: { user }, error, } = await supabase.auth.getUser(token);
        if (!user) {
...
export const editReading = async (req, res) => {
    const { id, mealTiming, timestamp, value, note } = req.body;
    const token = req.headers.authorization?.replace("Bearer ", "");
    try {
        const { data: { user }, error, } = await supabase.auth.getUser(token);
        if (!user) {
...
export const shareReportWithEmail = async (req, res) => {
    const file = req.file;
