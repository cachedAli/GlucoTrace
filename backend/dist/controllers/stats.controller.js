...
import { getResetStat } from '../utils/resetStat.js';
import { startOfWeek, differenceInDays } from "date-fns";
// Get previous stat
export const getPreviousStat = async (req, res) => {
...
export const archiveStats = async (req, res) => {
    const token = req.headers.authorization?.replace('Bearer ', '');
    try {
        const { data: { user }, error } = await supabase.auth.getUser(token);
        if (!user || error) {
